...
- `agent/src/api/`: FastAPI (`src.api.app:app`). Queues jobs and reads results; never runs a
  ...
  over the same jobs. `api/voice/`: `/v1/audio/transcriptions`, `/v1/audio/speech` and
  `/v1/voice/turns` over the `speech` service. The user comes from `deps.current_user` (the gateway's header, checked
...
- `agent/Dockerfile`, `db/`, `gateway/`, `tailscale/`, `backup/` and the root `docker-compose.yaml`:
  the Spark stack (gateway, api, worker, broker, db, anythingllm, vllm, speech, backup, tailscale). The
